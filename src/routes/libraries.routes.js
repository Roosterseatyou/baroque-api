...
router.get(
  ...
  authenticate,
  requireOrgRole(),
  librariesController.getLibraries,
);
...
router.post(
  "/:libraryId/import",
  authenticate,
  requireOrgOrLibraryRole(["owner", "admin", "editor"]),
  librariesController.importFromSpreadsheet,
);
...
router.get(
  ...
  authenticate,
  requireOrgOrLibraryRole(),
  librariesController.findLibraryDuplicates,
);

// Debug: return raw duplicates payload (no normalization) - requires org/library role
router.get(
  "/:libraryId/duplicates/raw",
  authenticate,
  requireOrgOrLibraryRole(),
  librariesController.findLibraryDuplicatesRaw,
);

// Force refresh duplicates (auth + role required)
router.post(
  "/:libraryId/duplicates/refresh",
  authenticate,
  requireOrgOrLibraryRole(["owner", "admin"]),
  librariesController.refreshLibraryDuplicates,
);

// Public duplicates endpoint (view-token)
router.get(
  "/public/:token/duplicates",
  librariesController.findLibraryDuplicatesPublic,
);

// View-token management (generate/revoke) - requires at least org or library owner/admin
router.post(
  "/:libraryId/view-token",
  authenticate,
  requireOrgOrLibraryRole(["owner", "admin"]),
  librariesController.generateViewToken,
);
router.delete(
  "/:libraryId/view-token",
  authenticate,
  requireOrgOrLibraryRole(["owner", "admin"]),
  librariesController.revokeViewToken,
);
