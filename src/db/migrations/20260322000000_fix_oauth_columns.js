...
export async function up(knex) {
  return knex.schema.alterTable('oauth_info', table => {
    ...
    try {
      ...
      console.warn('alter oauth token columns:', e && e.message ? e.message : e);
    }
...
export async function down(knex) {
  return knex.schema.alterTable('oauth_info', table => {
    try { table.string('access_token').nullable().alter(); } catch (e) {}
    try { table.string('refresh_token').nullable().alter(); } catch (e) {}
    try { table.string('provider_user_id').notNullable().alter(); } catch (e) {}
    try { table.string('avatar_url').notNullable().alter(); } catch (e) {}
    try { table.dropColumn('token_expires_at'); } catch (e) {}
  });
}
