...
export async function up(knex) {
  ...
  try {
    ...
    if (rows && rows.length) {
      ...
      await knex.raw(`ALTER TABLE libraries DROP FOREIGN KEY \`${constraintName}\``);
    }
  } catch (e) {
  ...
  try {
    ...
    if (rowsPg && rowsPg.length) {
      ...
      await knex.raw(`ALTER TABLE libraries DROP CONSTRAINT \"${constraintName}\"`);
    }
  } catch (e) {
  ...
  try {
    ...
    try {
      await knex.raw("ALTER TABLE libraries MODIFY organization_id CHAR(36) NOT NULL");
    } catch (e2) {
