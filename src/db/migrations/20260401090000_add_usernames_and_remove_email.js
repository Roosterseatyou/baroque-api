...
export async function up(knex) {
  ...
  const hasDiscriminator = await knex.schema.hasColumn('users', 'discriminator');
  if (!hasUsername || !hasDiscriminator) {
    ...
    for (const u of users) {
      ...
      for (let attempt = 0; attempt < 1000; attempt++) {
        const candidateDisc = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
        const exists = await knex('users')
          .whereRaw('LOWER(username) = LOWER(?) AND discriminator = ?', [base, candidateDisc])
          .first();
        if (!exists) { disc = candidateDisc; break; }
        // if base collides often, slightly alter base after some attempts
        if (attempt % 50 === 0) base = (base + Math.floor(Math.random() * 9)).slice(0, 30);
      }
      if (!disc) disc = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
...
export async function down(knex) {
  ...
  if (hasUsername || hasDiscriminator) {
    await knex.schema.alterTable('users', (table) => {
      try { table.dropUnique(['username', 'discriminator'], 'users_username_discriminator_unique'); } catch (e) {}
