...
export function requireOrgRole(requiredRole = null) {
  return async function (req, res, next) {
    ...
    let orgId =
      params.organizationId || params.orgId || params.id || body.organizationId;
    ...
    try {
      ...
      console.error(
        "Error deriving organizationId in middleware:",
    ...
    if (!orgId)
      return res
        .status(400)
        .json({ message: "organizationId parameter missing" });
    try {
      ...
      if (!membership) {
        console.debug(
          `Access denied: no membership. userId=${userId} orgId=${orgId}`,
        );
        return res
          .status(403)
          .json({ message: "Access denied: No membership found" });
      }
      ...
      if (requiredRole) {
        if (Array.isArray(requiredRole)) {
          if (!requiredRole.includes(userRole)) {
            console.debug(
              `Access denied: insufficient role. userId=${userId} orgId=${orgId} userRole=${userRole} required=${JSON.stringify(requiredRole)}`,
            );
            return res
              .status(403)
          ...
          if (userRole !== requiredRole) {
            console.debug(
              `Access denied: insufficient role. userId=${userId} orgId=${orgId} userRole=${userRole} required=${requiredRole}`,
            );
            return res
              .status(403)
              .json({ message: "Access denied: Insufficient role" });
      ...
      if (!allowedRoles.includes(userRole)) {
        console.debug(
          `Access denied: insufficient role. userId=${userId} orgId=${orgId} userRole=${userRole} allowed=${JSON.stringify(allowedRoles)}`,
        );
        return res
          .status(403)
          .json({ message: "Access denied: Insufficient role" });
...
export function requireLibraryRole(requiredRole = null) {
  return async function (req, res, next) {
    ...
    try {
      ...
      console.error(
        "Error deriving libraryId in middleware:",
        err && err.stack ? err.stack : err,
      );
    }
    if (!userId) return res.status(401).json({ message: "Unauthorized" });
    if (!libraryId)
      return res.status(400).json({ message: "libraryId parameter missing" });
    try {
      ...
      if (!membership) {
        console.debug(
          `Library access denied: no membership. userId=${userId} libraryId=${libraryId}`,
        );
        return res
          .status(403)
          .json({ message: "Access denied: No membership found" });
      ...
      if (requiredRole) {
        if (Array.isArray(requiredRole)) {
          if (!requiredRole.includes(userRole)) {
            console.debug(
              `Library access denied: insufficient role. userId=${userId} libraryId=${libraryId} userRole=${userRole} required=${JSON.stringify(requiredRole)}`,
            );
            return res
              .status(403)
              .json({ message: "Access denied: Insufficient role" });
          ...
          if (userRole !== requiredRole) {
            console.debug(
              `Library access denied: insufficient role. userId=${userId} libraryId=${libraryId} userRole=${userRole} required=${requiredRole}`,
            );
            return res
              .status(403)
              .json({ message: "Access denied: Insufficient role" });
      ...
      if (!allowedRoles.includes(userRole)) {
        console.debug(
          `Library access denied: insufficient role. userId=${userId} libraryId=${libraryId} userRole=${userRole} allowed=${JSON.stringify(allowedRoles)}`,
        );
        return res
          .status(403)
          .json({ message: "Access denied: Insufficient role" });
...
export function requireOrgOrLibraryRole(requiredRole = null) {
  return async function (req, res, next) {
    ...
    try {
      ...
      console.error(
        "Error deriving ids in requireOrgOrLibraryRole:",
        err && err.stack ? err.stack : err,
      );
    }
    ...
    try {
      ...
      if (!orgId)
        return res
          .status(400)
          .json({ message: "organizationId parameter missing" });
      const orgMembership = await db("organization_memberships")
        .where({ user_id: userId, organization_id: orgId })
        .first();
      if (!orgMembership) {
        const body = { message: "Access denied: No membership found" };
        if (process.env.NODE_ENV !== "production")
          body.debug = { userId, orgId, libraryId };
        return res.status(403).json(body);
      }
      const orgRole = orgMembership.role;
      if (!requiredRole) return next();
      if (Array.isArray(requiredRole)) {
        if (!requiredRole.includes(orgRole))
          return res
            .status(403)
            .json({ message: "Access denied: Insufficient role" });
        return next();
      ...
      console.error(
        "Error in requireOrgOrLibraryRole:",
        err && err.stack ? err.stack : err,
      );
      return res.status(500).json({ message: "Internal server error" });
    }
