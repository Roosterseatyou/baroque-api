...
function validatePiecePayload(data) {
  ...
  if (title && title.length > MAX_LEN.title)
    errors.push(`title must be <= ${MAX_LEN.title} characters`);
  if (composer && composer.length > MAX_LEN.composer)
    errors.push(`composer must be <= ${MAX_LEN.composer} characters`);
  if (data.arranger && data.arranger.length > MAX_LEN.arranger)
    errors.push(`arranger must be <= ${MAX_LEN.arranger} characters`);
  if (
    data.instrumentation &&
    data.instrumentation.length > MAX_LEN.instrumentation
  )
    errors.push(
      `instrumentation must be <= ${MAX_LEN.instrumentation} characters`,
    );
  if (data.publisher && data.publisher.length > MAX_LEN.publisher)
    errors.push(`publisher must be <= ${MAX_LEN.publisher} characters`);
  if (
    data.library_number &&
    data.library_number.length > MAX_LEN.library_number
  )
    errors.push(
      `library_number must be <= ${MAX_LEN.library_number} characters`,
    );
  if (data.difficulty && !ALLOWED_DIFFICULTIES.includes(data.difficulty))
    errors.push("difficulty value is invalid");
  if (data.quantity !== undefined) {
    const q = Number(data.quantity);
    if (!Number.isFinite(q) || q < 0 || q > MAX_QUANTITY)
      errors.push("quantity value is invalid");
  }
  return errors;
}
  ...
  try {
    ...
    try {
      if (req.user && req.user.id && piece.library_id) {
        membership = await librariesService.getMembershipForUser(
          piece.library_id,
          req.user.id,
        );
      }
    ...
    try {
      if (!membership && req.user && req.user.id && piece.library_id) {
        ...
        if (lib && lib.organization_id) {
          membership = await orgsService.getMembershipForUser(
            lib.organization_id,
            req.user.id,
          );
        }
...
export async function getPieces(req, res) {
  try {
    ...
    const page =
      req.query && (req.query.page || req.query.per_page || req.query.perPage);
    if (page) {
      ...
      const sortDir =
        ...
        "asc";
      // parse expanded_collections query parameter (comma-separated ids)
      const expandedRaw =
        req.query.expanded_collections || req.query.expandedCollections || "";
      const expanded = Array.isArray(expandedRaw)
        ? expandedRaw
        : expandedRaw
          ? String(expandedRaw)
              .split(",")
              ...
              .filter(Boolean)
          : [];
      const result = await piecesService.getPiecesPaged(req.params.libraryId, {
        page: pageNum,
        perPage,
        sortField,
        sortDir,
...
export async function updatePiece(req, res) {
  try {
    ...
    const errors = validatePiecePayload(data);
    if (errors.length) return res.status(400).json({ errors });
    const updatedPiece = await piecesService.updatePiece(
      req.params.pieceId,
      data,
    );
...
export async function searchPieces(req, res) {
  try {
    ...
    const byTitle =
      req.query.byTitle === "true" ||
      req.query.byTitle === "1" ||
      req.query.byTitle === "on" ||
      req.query.byTitle === true;
    const byComposer =
      req.query.byComposer === "true" ||
      req.query.byComposer === "1" ||
      req.query.byComposer === "on" ||
      req.query.byComposer === true;
    const byLibNumber =
      req.query.byLibNumber === "true" ||
      req.query.byLibNumber === "1" ||
      req.query.byLibNumber === "on" ||
      req.query.byLibNumber === true;
    const includeExternal =
      req.query.includeExternal === "true" ||
      req.query.includeExternal === "1" ||
      req.query.includeExternal === "on" ||
      req.query.includeExternal === true;
    ...
    if (includeExternal) {
      ...
      pieces = await piecesService.searchPiecesInOrgLibraries({
        libraryId: req.params.libraryId,
        orgId,
        query,
        maxResults: 200,
      ...
      pieces = await piecesService.searchPieces(req.params.libraryId, query, {
        byTitle,
        byComposer,
        byLibNumber,
      });
...
export async function autofillFromOrgLibraries(req, res) {
  try {
    ...
    if (!orgId)
      return res
        .status(400)
        .json({ error: "library not associated with an organization" });
    ...
    const results = await piecesService.searchPiecesInOrgLibraries({
      libraryId,
      orgId,
      query,
      maxResults: 20,
