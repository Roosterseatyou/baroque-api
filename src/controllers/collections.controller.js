...
export async function listCollections(req, res) {
  try {
    ...
    if (q) {
      ...
      rows = all.filter(
        (c) =>
          (c.name && c.name.toLowerCase().includes(qlow)) ||
          (c.library_number &&
            String(c.library_number).toLowerCase().includes(qlow)),
      );
    } else {
    ...
    try {
      ...
      console.error("Failed to serialize collections response", serr, {
        libraryId,
        sampleRow: rows && rows[0],
      });
      return res
        .status(500)
        .json({ error: "Failed to serialize collections response" });
    }
...
export async function updateCollection(req, res) {
  try {
    const id = req.params.collectionId;
    const { name, library_number, metadata } = req.body || {};
    const updated = await collectionsService.updateCollection(id, {
      name,
      library_number,
      metadata,
    });
