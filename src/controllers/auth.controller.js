...
export async function login(req, res) {
  try {
    ...
    const { token, refreshToken, refresh_expires_at, user } =
    ...
    if (process.env.DEBUG_OAUTH === "true") {
      debug.debugLog("auth.login: setting refresh cookie", {
        name: REFRESH_COOKIE_NAME,
        valueSnippet: (refreshToken || "").slice(0, 8) + "...",
        options: cookieOpts,
      });
    }
    ...
    const resp = {
      token,
      refresh_expires_at,
      user: { ...respUser, email: user.email || null },
    };
    if (typeof user?.restored !== "undefined")
      resp.restored = user.restored === true;
...
export async function refresh(req, res) {
  try {
    ...
    if (!refreshToken)
      return res.status(400).json({ error: "refresh token cookie missing" });
    const metadata = { ip: req.ip, userAgent: req.get("user-agent") };
    const result = await authService.exchangeRefreshToken(
      refreshToken,
      true /* rotate */,
      metadata,
    );
    ...
    const cookieOpts = buildRefreshCookieOptions(FRONTEND_ORIGIN);
    if (process.env.DEBUG_OAUTH === "true") {
      debug.debugLog("auth.refresh: setting refresh cookie (rotated)", {
        name: REFRESH_COOKIE_NAME,
        valueSnippet: (result.refreshToken || "").slice(0, 8) + "...",
        options: cookieOpts,
      });
    }
    ...
    if (result.restored === true || result.user?.restored === true)
      resp.restored = true;
    res.status(200).json(resp);
  } catch (error) {
    console.error(
...
export async function googleCallback(req, res) {
  try {
    debug.debugLog("auth.googleCallback invoked:", {
      url: req.originalUrl,
      path: req.path,
      cookiesHeader:
        req.headers && req.headers.cookie ? req.headers.cookie : null,
    });
      ...
      if (rawCookies) {
        ...
        while ((m = regex.exec(rawCookies)) !== null) {
          ...
          try {
            ...
            if (process.env.DEBUG_OAUTH === "true") {
              debug.debugLog(
                "auth.googleCallback: scanned oauth_state candidate",
                {
                  rawVal,
                  norm,
                  decoded,
                  candidateState:
                    candidate && candidate.state ? candidate.state : null,
                },
              );
            }
            if (candidate && candidate.state === state) {
              parsed = candidate;
              break;
            }
          } catch (e) {
    ...
    if (!parsed || parsed.state !== state) {
      ...
      try {
        ...
        if (record) {
          ...
          // best-effort cleanup
          try {
            await knex("oauth_states").where({ state }).del();
          } catch (e) {
            /* ignore */
          }
        }
        ...
        if (process.env.DEBUG_OAUTH === "true")
          debug.debugLog("auth.googleCallback: oauth_states DB lookup failed", {
    ...
    if (!parsed || parsed.state !== state) {
      if (process.env.DEBUG_OAUTH === "true") {
        debug.debugLog("auth.googleCallback: oauth state mismatch or missing", {
          expectedState: state,
    ...
    const FRONTEND_ORIGIN_CB =
      process.env.FRONTEND_ORIGIN || "http://localhost:5173";
    const cookieOpts = buildRefreshCookieOptions(FRONTEND_ORIGIN_CB);
    if (process.env.DEBUG_OAUTH === "true") {
      debug.debugLog(
        "auth.googleCallback: setting refresh cookie from google callback",
        {
          name: REFRESH_COOKIE_NAME,
          valueSnippet: (refresh.token || "").slice(0, 8) + "...",
          options: cookieOpts,
        },
      );
    }
...
export async function revokeSession(req, res) {
  try {
    ...
    if (!ok)
      return res
        .status(404)
        .json({ error: "session not found or not owned by user" });
    res.status(204).send();
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
