...
export async function getTags(req, res) {
    try {
        ...
        console.error('Error in getTags:', error && error.stack ? error.stack : error);
...
export async function getTagsForPiece(req, res) {
    try {
        ...
        console.error('Error in getTagsForPiece:', error && error.stack ? error.stack : error);
        res.status(500).json({ message: 'Internal server error' });
    }
}
...
export async function updateTag(req, res) {
    try {
        ...
        const tag = await tagsService.updateTag({ libraryId, tagId, name: name.trim() });
