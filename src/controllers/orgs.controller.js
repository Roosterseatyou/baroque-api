...
export async function createOrganization(req, res) {
  try {
    ...
    const {
      ...
      name: orgName,
      defaultLibrary,
    } = await orgsService.createOrganization({ userId: req.user.id, name });
...
export async function getOrganization(req, res) {
  try {
    const organization = await orgsService.getOrganizationById(
...
export async function addMember(req, res) {
  try {
    ...
    if (userId) {
      ...
      const requesterMembership = await orgsService.getMembershipForUser(
        req.params.organizationId,
        requesterId,
      );
      if (
      ...
      const existing = await orgsService.getMembershipForUser(
        req.params.organizationId,
        userId,
      );
      if (existing)
    ...
    if (!username)
      return res.status(400).json({ error: "username required to invite" });
...
export async function getMembers(req, res) {
  try {
    const members = await orgsService.getOrganizationMembers(
      req.params.organizationId,
    );
...
export async function removeMember(req, res) {
  try {
    ...
    const targetMembership = await orgsService.getMembershipForUser(
      req.params.organizationId,
      userId,
    );
    if (!targetMembership)
...
export async function updateOrganization(req, res) {
  try {
    ...
    if (typeof allow_sharing !== "undefined") {
      ...
      const membership = await orgsService.getMembershipForUser(
        req.params.organizationId,
        req.user && req.user.id,
      );
      if (!membership || !["owner", "admin"].includes(membership.role))
        return res.status(403).json({ error: "Forbidden" });
      await orgsService.updateOrganizationSettings(req.params.organizationId, {
        allow_sharing: !!allow_sharing,
      });
    }
    const latest = await orgsService.getOrganizationById(
      req.params.organizationId,
    );
  ...
  try {
    ...
    try {
      ...
      const audit = await knex("audit_logs")
        ...
        .andWhere(function () {
          this.where({ performed_by: requesterId }).orWhere(
            "details",
            "like",
            `%${requesterId}%`,
          );
