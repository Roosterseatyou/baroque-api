...
export async function addComment(req, res) {
  try {
    ...
    if (!text) {
      console.debug(`addComment validation failed: empty content, user=${userId} piece=${pieceId} body=${JSON.stringify(req.body)}`);
      return res.status(400).json({ error: 'Content required' });
    }
    if (text.length > 250) {
      console.debug(`addComment validation failed: content too long (${text.length}), user=${userId} piece=${pieceId} body=${JSON.stringify(req.body).slice(0,200)}`);
    ...
    try {
      if (orgId && userId) {
        membership = await db('organization_memberships').where({ organization_id: orgId, user_id: userId }).first();
      }
    } catch (e) {
      console.error('addComment: error looking up membership', e && e.stack ? e.stack : e);
    }

    const role = membership?.role || null;
    // allow viewers to post top-level comments as requested

    const comment = await commentsService.addComment({ pieceId, userId, content: text, parentId: parentId || null });
...
export async function updateComment(req, res) {
  try {
    ...
    if (text.length > 250) return res.status(400).json({ error: 'Content exceeds 250 characters' });

    const existing = await db('piece_comments').where({ id: commentId }).first();
    ...
    const membership = await db('organization_memberships').where({ organization_id: orgId, user_id: userId }).first();
    const userRole = membership?.role;
    if (existing.user_id !== userId && !['owner','admin','manager'].includes(userRole)) {
      return res.status(403).json({ error: 'Insufficient permissions to edit comment' });
    }

    const updated = await commentsService.updateComment(commentId, userId, { content: text });
