...
export async function scrapeByQuery(req, res) {
    try {
        ...
        if (!title && !composer) return res.status(400).json({ error: 'title or composer required' });
        const results = await scrapeService.scrapeByQuery({ title, composer, maxResults: maxResults || 3 });
