...
export async function authorize(req, res) {
  try {
    ...
    // persist server-side
    await knex("oauth_states").insert({
      state,
      user_id: user.id,
...
export async function callback(req, res) {
  try {
    debug.debugLog("google.callback invoked:", {
      url: req.originalUrl,
      path: req.path,
      cookiesHeader:
    ...
    if (!userId && email) {
      try {
        ...
        if (userRow) {
          userId = userRow.id;
          debug.debugLog(
            "google.callback: mapped to user by username base from email",
            base,
            userId,
          );
        } else {
          console.warn(
            "google.callback: no local user for google email-derived username",
            base,
          );
        }
      } catch (e) {
        console.warn(
          "google.callback: failed to lookup user by derived username",
          e && e.message,
        );
      }
    }
    ...
    if (!userId && process.env.ALLOW_GOOGLE_SIGNUP === "true") {
      try {
        ...
        if (created && created.id) {
          userId = created.id;
          debug.debugLog(
            "google.callback: created local user for google sign-up",
            userId,
          );
        }
      } catch (e) {
        console.warn(
          "google.callback: failed to create local user from google profile",
          e && e.message,
        );
      }
    }
    ...
    if (userId) {
      const existing = await knex("oauth_info")
        .where({ user_id: userId, provider: "google" })
        .first();
      if (existing) {
        await knex("oauth_info")
      ...
      try {
        ...
        try {
          ...
          if (process.env.DEBUG_OAUTH === "true") {
            debug.debugLog("google.callback: setting refresh cookie", {
        ...
        if (process.env.DEBUG_OAUTH === "true") {
          debug.debugLog(
            "google.callback: set refresh cookie for user",
            userId,
          );
        }
      } catch (e) {
        console.warn(
          "Failed to create server refresh token after google callback",
          e && e.message,
        );
      }
    } else {
      console.warn(
        "google.callback: userId still unknown; tokens will not be persisted to oauth_info",
      );
    }
...
export async function status(req, res) {
  try {
    ...
    const row = await knex("oauth_info")
      .where({ user_id: userId, provider: "google" })
      .first();
...
export async function debugInfo(req, res) {
  try {
    ...
    const row = await knex("oauth_info")
      ...
      .first();
    if (!row)
      return res.status(404).json({ error: "No google oauth info for user" });
    return res.json({
      id: row.id,
      user_id: row.user_id,
      email: row.email,
...
export async function tokenInfo(req, res) {
  try {
    ...
    if (!resp.ok) {
      ...
      return res
        .status(resp.status)
        .json({ error: `tokeninfo returned ${resp.status}`, body: text });
    }
...
export async function listOauthStates(req, res) {
  try {
    if (process.env.NODE_ENV === "production")
      return res.status(404).json({ error: "Not found" });
    const rows = await knex("oauth_states")
      .select("state", "user_id", "redirect_to", "created_at")
