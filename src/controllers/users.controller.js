...
export async function updateUserProfile(req, res) {
    try {
        const { name, username, discriminator } = req.body;
        const updatedUser = await usersService.updateUserProfile(req.user.id, { name, username, discriminator });
...
export async function restoreUser(req, res) {
    try {
        ...
        if (targetUserId !== requesterId && !adminSecret) return res.status(403).json({ error: 'Forbidden' });
        // If admin secret provided, accept it (no verification beyond presence). For stricter control, compare to env ADMIN_SECRET.
        if (adminSecret && process.env.ADMIN_SECRET && adminSecret !== process.env.ADMIN_SECRET) return res.status(403).json({ error: 'Forbidden' });
...
export async function getDeletedOrganizations(req, res) {
    try {
        const organizations = await usersService.getDeletedOrganizationsForUser(req.user.id);
        res.status(200).json(organizations);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
