...
function _serverNormalizePayload(payload, opts = {}) {
  ...
  if (Array.isArray(payload))
    return { groups: payload, cachedAt: null, scanning: false };
  ...
  if (payload && payload.id && payload.result !== undefined) {
    payload = {
      ...
      cachedAt: payload.cached_at || payload.cachedAt,
      scanning: payload.status === "pending" || payload.status === "running",
    };
  }
  ...
  out.groups = raw
    .map((g) => {
      ...
      parsedPieces = parsedPieces.filter(
        (pp) =>
      ...
      parsedPieces = parsedPieces.filter((pp) => {
        const key =
          pp.id || `libnum:${pp.library_number}` || JSON.stringify(pp);
      ...
      const titleExample =
        g.titleExample ||
        g.title_example ||
        (parsedPieces[0] ? parsedPieces[0].title : "") ||
        "";
      return {
        titleKey: g.titleKey || g.title_example || titleExample,
        titleExample,
  ...
  let filtered = out.groups.filter((g) => {
    ...
    const hasMeaning = g.pieces.some(
      (pp) =>
        (pp.title && pp.title.length) ||
        (pp.composer && pp.composer.length) ||
        (pp.library_number && String(pp.library_number).length),
  ...
  function extractLibNum(g) {
    ...
    try {
      ...
      if (g && Array.isArray(g.pieces) && g.pieces.length > 0) {
        const ln =
          g.pieces[0].library_number ||
          g.pieces[0].libraryNumber ||
          g.pieces[0].lib_number;
  ...
  for (const k of Object.keys(buckets)) {
    buckets[k].sort((a, b) => {
      ...
      if (b.pieces.length - a.pieces.length !== 0)
        return b.pieces.length - a.pieces.length;
      return String(a.titleExample || "").localeCompare(
        String(b.titleExample || ""),
      );
    });
  }
  ...
  const lowInfoCount = combined.filter(
    (g) =>
      !g.titleExample ||
      !g.titleExample.trim() ||
      (Array.isArray(g.pieces) && g.pieces.length < 2),
  ).length;
  ...
  const shouldLog = (() => {
    ...
    if (now - last > DUP_LOG_TTL_MS) {
      _recentDupLogAt.set(libId, now);
      return true;
    }
    return false;
  })();
  if (total > 200 || (total > 0 && lowInfoCount / total > 0.8)) {
    if (shouldLog)
      console.warn(
        `Duplicate scan returned suspiciously large/low-info payload (groups=${total}, lowInfo=${lowInfoCount}); returning paginated/truncated payload instead of empty scanning state`,
      );
  }
  ...
  try {
    if (shouldLog && Array.isArray(out.groups) && out.groups.length > 0) {
      const sample = out.groups
        .slice(0, 3)
        .map((g) => ({
          titleExample: g.titleExample,
          piecesLen: Array.isArray(g.pieces) ? g.pieces.length : 0,
          severity: g.severity,
        }));
  ...
  try {
    ...
    try {
      if (req.user && req.user.id) {
        membership = await libraryService.getMembershipForUser(
...
export async function importFromSpreadsheet(req, res) {
  try {
    ...
    try {
      ...
      if (clientImportId) {
        const cacheKey = _makeImportCacheKey(
          userIdLog,
          libraryId,
          clientImportId,
        );
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Skip rows that are completely empty (common as visual separators in exported CSVs).
      // These should be ignored rather than treated as errors.
      // Treat rows as empty only if all non-meta fields are empty. Ignore
      // any meta fields that start with '_' (for example __rowIndex set by client).
      const isEmptyRow = Object.keys(r)
      ...
      // Determine whether the row appears to describe a piece and/or a collection.
      // Only treat a row as a collection (or collection header) if a collection name is present.
      // Some CSVs use the "Lib #" column for piece library numbers when collection is blank;
      // treating collection_library_number alone as a collection causes a collection to be created
      // for every piece that has a library number. So require collection_name to be present.
      const hasCollection =
        !!r.collection_name && String(r.collection_name).trim();
      const hasTitle = !!r.title && String(r.title).trim();
      const hasComposer = !!r.composer && String(r.composer).trim();
      const hasPieceFields = hasTitle && hasComposer;

      // If the row has neither piece nor collection information, it's invalid.
      if (!hasPieceFields && !hasCollection) {
        results.failed++;
        results.errors.push({
          row: i + 1,
          error:
            "Missing title and composer (or provide collection_name/library number)",
        });
        continue;
      }

      // If the row is collection-only (no piece fields), try to find/create the collection and continue.
      if (hasCollection && !hasPieceFields) {
        try {
          // Attempt to find an existing collection by library_number and/or name
          let found = null;
          const cols =
            await collectionsService.getCollectionsForLibrary(libraryId);
          if (cols && cols.length) {
            found =
              cols.find((c) => {
                const colLibNum =
                  r.collection_library_number &&
                  String(r.collection_library_number).trim();
                const colName =
                  r.collection_name &&
                  String(r.collection_name).trim().toLowerCase();
                const cLibNum = String(c.library_number || "").trim();
                const cName = String(c.name || "")
                  .trim()
                  .toLowerCase();
                // If both lib num and name provided, match both. Otherwise match whichever was provided.
                if (colLibNum && colName)
                  return cLibNum === colLibNum && cName === colName;
                if (colLibNum) return cLibNum === colLibNum;
                if (colName) return cName === colName;
                return false;
              }) || null;
          }
          if (!found) {
            // create a new collection (use provided library_number if any)
            const createPayload = {
              name: r.collection_name || "Unnamed collection",
            };
            if (r.collection_library_number)
              createPayload.library_number = r.collection_library_number;
            await collectionsService.createCollection({
              libraryId,
              name: createPayload.name,
              library_number: createPayload.library_number || null,
              metadata: {},
            });
            // We count creating a collection as a successful created row.
            results.created++;
          } else {
            // If collection found, consider this row processed successfully.
            results.created++;
          }
        } catch (e) {
          results.failed++;
          results.errors.push({
            row: i + 1,
            error: "Collection create/find failed: " + (e.message || String(e)),
          });
        }
        // move to next row; do not attempt to create a piece when piece fields are absent
        continue;
      }

      try {
        // If a collection name is present, find or create the collection first
        // Note: do not treat a bare collection_library_number (without name) as a collection
        if (r.collection_name) {
          try {
            ...
            if (!found) {
              ...
              r.collection_id = created.id;
            } else {
              r.collection_id = found.id;
            }
          } catch (e) {
            // if collection creation failed, record an error for this row and skip
            results.failed++;
            results.errors.push({
              row: i + 1,
              error:
                "Collection create/find failed: " + (e.message || String(e)),
            });
            continue;
          }
        }
        ...
        if (r.tags && String(r.tags).trim()) {
          ...
          for (const name of tagNames) {
            ...
            await tagsService.attachTagToPiece({
              pieceId: piece.id,
              tagId: tag.id,
            });
          }
    ...
    try {
      ...
      if (cacheKey)
        _recentImportCache.set(cacheKey, { ts: Date.now(), result: results });
    } catch (cacheErr) {
...
export async function findLibraryDuplicates(req, res) {
  try {
    ...
    const perPage =
      Number(req.query?.per_page || req.query?.perPage) || undefined;
    const payload =
      await piecesService.findDuplicatesInLibraryCached(libraryId);
    // normalize payload shape before returning to clients (supports pagination)
    return res
      .status(200)
      .json(_serverNormalizePayload(payload, { page, perPage, libraryId }));
  } catch (err) {
...
export async function findLibraryDuplicatesPublic(req, res) {
  try {
    ...
    return res
      .status(200)
      .json(
        _serverNormalizePayload(payload, { page, perPage, libraryId: lib.id }),
      );
  } catch (err) {
...
export async function refreshLibraryDuplicates(req, res) {
  try {
    ...
    return res
      .status(202)
      .json(_serverNormalizePayload(payload, { page: 1, perPage: 50 }));
  } catch (err) {
