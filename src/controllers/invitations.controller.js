...
export async function createInvite(req, res) {
  try {
    ...
    const invite = await invitationsService.createInvitation({
      ...
      invitedUsername: invitedUsernameToStore,
      invitedDiscriminator: invitedDiscriminatorToStore,
      invitedUserId,
      ...
      if (username) {
        ...
        if (currentUser && currentUser.discriminator) {
          this.orWhere(function () {
            this.whereRaw("LOWER(i.invited_username) = LOWER(?)", [
              username,
        ...
        this.orWhere(function () {
          this.whereRaw("LOWER(i.invited_username) = LOWER(?)", [
            username,
          ]).andWhere("i.invited_discriminator", null);
        });
      }
    });
...
export async function respondInvite(req, res) {
  try {
    ...
    await knex.transaction(async (trx) => {
      ...
      const invite = await trx("invitations")
        .where({ id: inviteId })
        .forUpdate()
        .first();
        ...
        if (invite.invited_discriminator) {
          ...
          if (
            currentUsername.toLowerCase() !==
              String(invite.invited_username).toLowerCase() ||
            currentUser.discriminator !== invite.invited_discriminator
          ) {
      ...
      if (action === "decline") {
        await trx("invitations")
          .where({ id: inviteId })
          .update({ status: "declined", updated_at: trx.fn.now() });
      ...
      if (!targetUserId && invite.invited_username) {
        ...
        if (invite.invited_discriminator) {
          const u = await trx("users")
            .whereRaw("LOWER(username)=LOWER(?)", [invite.invited_username])
            .andWhere({ discriminator: invite.invited_discriminator })
            .first();
      ...
      const existing = await trx("organization_memberships")
        .where({
    ...
    if (err && err.message === "Invitation not found")
