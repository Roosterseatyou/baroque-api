...
export async function generateViewToken(libraryId) {
    ...
    await db('libraries').where({ id: libraryId }).update({ view_token: token, view_token_created_at: db.fn.now() });
    return token;
}

export async function revokeViewToken(libraryId) {
    await db('libraries').where({ id: libraryId }).update({ view_token: null, view_token_created_at: null });
    return true;
}
