...
export async function scrapeByQuery({ title, composer, maxResults = 3 }) {
    ...
    const keywords = encodeURIComponent(((title || '') + ' ' + (composer || '')).trim());
    const searchUrl = `https://www.halleonard.com/search/search.action?keywords=${keywords}&dt=item`;

    let $;
    try {
        // fetch search page
        const res = await axios.get(searchUrl, { headers: { 'User-Agent': USER_AGENT }, timeout: 10000 });
        $ = load(res.data);
    } catch (error) {
        ...
        const baseMessage = 'Unexpected error while fetching Hal Leonard search results';
