...
export async function createOrganization({ userId, name }) {
  ...
  return {
    id: organizationId,
    name,
    defaultLibrary: { id: libraryId, name: defaultLibraryName },
  };
}
...
export async function updateOrganizationSettings(
  ...
  if (typeof allow_sharing !== "undefined")
    update.allow_sharing = allow_sharing;
...
export async function deleteOrganization(organizationId, performedBy = null) {
  ...
  await import("./audit.service.js").then((a) =>
    a.writeAudit({
      entityType: "organization",
      entityId: organizationId,
      action: "organization_soft_deleted",
      details: orgName
        ? { name: orgName, scheduled_days: days }
        : { scheduled_days: days },
...
export async function scheduleOrganizationHardDelete(
  organizationId,
  when,
  performedBy = null,
) {
...
export async function hardDeleteOrganization(
  ...
  try {
    ...
    await q("piece_tags")
      .whereIn(
        "piece_id",
        q("pieces").whereIn("library_id", libIdsQuery).select("id"),
      )
      .del();
    // Delete piece_comments for pieces in these libraries
    await q("piece_comments")
      .whereIn(
        "piece_id",
        q("pieces").whereIn("library_id", libIdsQuery).select("id"),
      )
    ...
    await q("piece_tags")
      .whereIn(
        "tag_id",
        q("tags").whereIn("library_id", libIdsQuery).select("id"),
      )
      .del();
    ...
    await q("organization_memberships")
      .where({ organization_id: organizationId })
      .del();
      ...
      a.writeAudit(
        {
          entityType: "organization",
          entityId: organizationId,
          action: "organization_deleted_hard",
          details: null,
          performedBy: performedBy || null,
        },
        trx,
