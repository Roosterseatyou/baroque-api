...
(async () => {
  try {
    ...
    console.log('[findDuplicatesWorker] finished for library', libraryId, 'duration_ms=', dur);
    // send result back to parent
    parentPort.postMessage({ success: true, groups });
  } catch (err) {
    parentPort.postMessage({ success: false, error: (err && err.stack) ? err.stack : String(err) });
  }
})();
