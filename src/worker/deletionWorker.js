    ...
    try {
      ...
      await knex.transaction(async (trx) => {
        ...
        const ownerRows = await trx("organization_memberships")
          .where({ user_id: r.id, role: "owner" })
          .select("organization_id");
        for (const or of ownerRows) {
          try {
            await orgsService.hardDeleteOrganization(
              or.organization_id,
              r.id,
              trx,
            );
            ...
            console.error(
              "[deletionWorker] failed to hard-delete org",
              or.organization_id,
              e,
            );
          }
        ...
        try {
          await trx("oauth_info").where({ user_id: r.id }).del();
        } catch (e) {
          void e;
        }
        try {
          await trx("refresh_tokens").where({ user_id: r.id }).del();
        } catch (e) {
          void e;
        }
        try {
          await trx("oauth_states").where({ user_id: r.id }).del();
        } catch (e) {
          void e;
        }
